...
#   stages: census_hr census_ws analyses d_cpu n1 i2 fresh sweep_ws sweep_hr sweep_ws_ext k_profile madnlp
set -u
cd "$(dirname "$0")/../.."
GPU="${1:-1}"; shift || true
...
stage_sweep_hr() {
# ---- run C: ILB-on batch sweep through the harness (same code path as the WS sweep; peak memory) ----
# max_solves = max(pool, 3N) so the hot-restart pipeline is full (same rule as the Aug-10 sweep)
for n in 125 250 500 1000 2000 4000 8000; do
  ms=$(( n*3 > 2000 ? n*3 : 2000 ))
  for v in 90 180; do abl "sweep_nav${v}_hr_b${n}" "$NAV/jaxipm_ablation_sector${v}_hr_ir0_b${n}_results.npz" --scenario nav --variants "$v" --mode hr --batch "$n" --max-solves "$ms" --repeats 3 --no-save-z; done
done
for n in 125 250 500 1000 2000; do
  ms=$(( n*3 > 1000 ? n*3 : 1000 ))
  for v in 1.0 1.5 2.0; do abl "sweep_track${v}_hr_b${n}" "$TRK/jaxipm_ablation_v${v}_hr_ir0_b${n}_results.npz" --scenario track --variants "$v" --mode hr --batch "$n" --max-solves "$ms" --repeats 3 --no-save-z; done
done
for n in 25 75 150 300 600 1200; do
  ms=$(( n*3 > 75 ? n*3 : 75 ))
  for v in 2 4; do abl "sweep_multi${v}_hr_b${n}" "$MUL/jaxipm_ablation_${v}_hr_ir0_b${n}_results.npz" --scenario multi --variants "$v" --mode hr --batch "$n" --max-solves "$ms" --repeats 3 --no-save-z; done
done
}

stage_sweep_ws_ext() {
# ---- run C: extend the ILB-off sweep towards OOM ----
for n in 4000 8000; do for v in 90 180; do abl "sweep_nav${v}_ws_b${n}" "$NAV/jaxipm_ablation_sector${v}_ws_ir0_b${n}_results.npz" --scenario nav --variants "$v" --mode ws --batch "$n" --repeats 3 --no-save-z; done; done
for n in 2000 4000; do for v in 1.0 1.5 2.0; do abl "sweep_track${v}_ws_b${n}" "$TRK/jaxipm_ablation_v${v}_ws_ir0_b${n}_results.npz" --scenario track --variants "$v" --mode ws --batch "$n" --repeats 3 --no-save-z; done; done
for n in 1200 2400; do for v in 2 4; do abl "sweep_multi${v}_ws_b${n}" "$MUL/jaxipm_ablation_${v}_ws_ir0_b${n}_results.npz" --scenario multi --variants "$v" --mode ws --batch "$n" --repeats 3 --no-save-z; done; done
}

# kprof <logname> <expected-json> <args...>
kprof() {
  local name="$1" expect="$2"; shift 2
  if [ -f "$expect" ]; then log "skip $name (exists: $expect)"; return; fi
  log ">>> $name : $*"
  "$PY" -m tests.rebuttal.k_fusion_profile "$@" --gpu "$GPU" > "$LOGD/$name.log" 2>&1 \
    && grep -E "^\[(wall|dev)\]|fusion tax|^saved" "$LOGD/$name.log" | sed 's/^/    /' \
    || { echo "    FAIL (exit $?) -- see $LOGD/$name.log"; tail -4 "$LOGD/$name.log" | sed 's/^/    /'; }
}
stage_k_profile() {
# ---- run K: fusion cost profile at the paper batch sizes (+ nav N=1) ----
R=tests/rebuttal/results
kprof "k_nav90_b500"   "$R/K_fusion_profile_nav_s90_b500.json"  --scenario nav --variants 90 --batch 500
kprof "k_nav90_b1"     "$R/K_fusion_profile_nav_s90_b1.json"    --scenario nav --variants 90 --batch 1 --warm-solves 4
kprof "k_track1.5_b250" "$R/K_fusion_profile_track_v1.5_b250.json" --scenario track --variants 1.5 --batch 250
kprof "k_multi2_b75"   "$R/K_fusion_profile_multi_q2_b75.json"   --scenario multi --variants 2 --batch 75
"$PY" -m tests.rebuttal.k_fusion_profile --analyze > "$LOGD/k_analyze.log" 2>&1 && log "K analysis done" || log "K analysis FAILED"
}
